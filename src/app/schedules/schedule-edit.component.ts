...
export class ScheduleEditComponent implements OnInit {
    ...
    loadScheduleDetails() {
        ...
        this.dataService.getScheduleDetails(this.id)
            .subscribe((schedule: IScheduleDetails) => {
                ...
                console.log('Date End '+new DateFormatPipe().transform(schedule.timeEnd, ['local']));
                // Convert date times to readable format
                this.schedule.timeStart = new DateFormatPipe().transform(schedule.timeStart, ['local']); // new DateFormatPipe().transform(schedule.timeStart, ['local']);
                this.schedule.timeEnd = new DateFormatPipe().transform(schedule.timeEnd, ['local']); //new DateFormatPipe().transform(schedule.timeEnd, ['local']);
    ...
    updateSchedule(editScheduleForm: NgForm) {
        ...
        this.loadingBarService.start();
        console.log('Update Schedule '+scheduleMapped);
        ...
        this.notificationService.openConfirmationDialog('Are you sure you want to remove '
            ...
            () => {
                ...
                this.dataService.deleteScheduleAttendee(this.schedule.id, attendee.id)
                    .subscribe(() => {
                        this.itemsService.removeItemFromArray<IUser>(this.schedule.attendeesUsers, attendee);
